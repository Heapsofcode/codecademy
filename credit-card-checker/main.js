...
const validateCred = (arr) => {
    let creditCardInt = arr.map(Number);
...
let results = findInvalidCards(batch);

const idInvalidCardCompanies = (results) => {
  invalidCardCompanies = [];
  uniqueCompanies = [];

  results.forEach((invalidNum) => {
    switch (invalidNum[0]) {
      case 3:
  ...
  invalidCardCompanies.forEach(element => {
    if (!uniqueCompanies.includes(element)) {
